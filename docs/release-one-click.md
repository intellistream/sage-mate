...
```bash
sage-mate-v4.6.0-linux.run
```

运行：

```bash
chmod +x sage-mate-v4.6.0-linux.run
./sage-mate-v4.6.0-linux.run
```
...
```bash
~/.local/share/sage-mate-installer/sage-mate-v4.6.0
```
...
./sage-mate-v4.6.0-linux.run --web-only

# 只安装本地 Sage Mate 代码编辑模式
./sage-mate-v4.6.0-linux.run --code-only

# 两者都装，但使用独立 checkout 和独立端口
./sage-mate-v4.6.0-linux.run --both
```
...
```bash
sage-mate-v4.6.0.tar.gz
```

解压后进入目录：

```bash
tar -xzf sage-mate-v4.6.0.tar.gz
cd sage-mate-v4.6.0
```
...
```bash
curl -fsSL https://github.com/intellistream/sage-mate/releases/download/v4.6.0/hosted-web.sh \
  -o /tmp/hosted-web.sh
